...
gulp.task('test', async() => {
    return gulp.src('app/modules/**/*.js')       // 源文件
        .pipe(uglify())           // 使用插件
        // .pipe(rename({
        //     suffix:".min"      // 给所有的文件名加上后缀.min
        //     }))
        .pipe(gulp.dest('dist/lib/app/modules')) // 设定输出目录
});
...
gulp.task('oms', async() => {
    return gulp.src('oms/**/*.js')       // 源文件
        .pipe(uglify())           // 使用插件
        // .pipe(rename({
        //     suffix:".min"      // 给所有的文件名加上后缀.min
        //     }))
        .pipe(gulp.dest('dist/lib/oms')) // 设定输出目录
});
...
gulp.task('all', gulp.series('clean', 'app-js', 'oms', 'config-js', 'cssmin', 'copy-html', 'copy-image1', 'copy-image2', 'copy-lib', function (done) {
    done();
}));

...
layui.define('layer', function (exports) {
    ...
    Class.prototype.initIE = function () {
        var that = this,
            ...
            iframe = $('<iframe id="' + ELEM_IFRAME + options.iframeIndex + '" class="' + ELEM_IFRAME + '" name="' + ELEM_IFRAME + options.iframeIndex + '" frameborder="0"></iframe>'),

            elemForm = $(['<form target="' + ELEM_IFRAME + options.iframeIndex + '" class="' + ELEM_FORM + '" method="post" key="set-mine" enctype="multipart/form-data" action="' + options.url + '">',
                '</form>'].join(''));

        //插入iframe    
        $('#' + ELEM_IFRAME + options.iframeIndex)[0] || $('body').append(iframe);
    ...
    Class.prototype.upload = function (files, type) {
        var that = this,
                ...
                layui.each(items, function (index, file) {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            successful++;
                            done(index, res, function () {
                                layer.close(Class.layerIndex);
                            });
                            allDone();
                        },
                        //异常回调
            ...
            //低版本IE处理方式，不支持跨域,
            iframeSend = function () {
                var iframe = $('#' + ELEM_IFRAME + options.iframeIndex);
                Class.layerIndex = layer.load(2, {
                    content: '文件上传中...',
                    shade: [0.1, '#000'],
                    success: function(layero) {
                ...
                // console.log(that.elemFile.prop('outerHTML'));
                // console.log('iframeHtml',that.elemFile.parent().prop('outerHTML'));
                ...
                $('iframe').load(function () {
                    ...
                    if (res) {
                            ...
                            // IE10以下无法通过file.value = '' 重置(通过拷贝元素实现)
                            var $cloneInputFile = that.elemFile.clone(true).val('');
                            that.elemFile.after($cloneInputFile);
                            that.elemFile.remove();

                            that.elemFile = $cloneInputFile;
            ...
            //统一回调,
            done = function (index, res, cb) {
                that.elemFile.next('.' + ELEM_CHOOSE).remove();
                elemFile.value = '';
                if (typeof res !== 'object') {
                    try {
                        // console.log('统一回调', res);
                        // JS中能精准表示的最大整数是 Math.pow(2, 53),十进制即9007199254740992,JSON.parse会引起精度丢失问题
                        var newResStr = res.replace(/\"fileId\":(\d+)/,'"fileId": "$1"');
                        res = JSON.parse(newResStr);
                    } catch (e) {
                        res = {};
            ...
            send = function () {
                ...
                //上传前的回调(判断文件个数是否超出)
                if (Object.prototype.toString.call(options.before) == '[object Function]') {
                    if (!options.before(args)) {
                        if (device.ie && device.ie < 10) {
                            // 上传文件个数限制为1， 上传完成后再上传提示最多上传一个文件，导致input file 存在值，而无法再上传同一文件
                            // IE10以下无法通过file.value = '' 重置(通过拷贝元素实现)
                            var $cloneInputFile = that.elemFile.clone(true).val('');
                            that.elemFile.after($cloneInputFile);
                            that.elemFile.remove();
        ...
        switch (options.accept) {
            case 'file': //一般文件
                if (exts && !RegExp('\\w\\.(' + exts + ')$', 'i').test(escape(value))) {
                    // console.log(exts.split('|').join('、'));
                    that.msg('选择的文件中包含不支持的格式,请上传【' + exts.split('|').join('、') + '】格式的文件');
            ...
            default: //图片文件
                ...
                if (check) {
                    that.msg('选择的图片中包含不支持的格式,请上传【' + exts.split('|').join('、') + '】格式的文件');
                    return elemFile.value = '';
                }
                break;
        }

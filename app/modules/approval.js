layui.define(function (exports) {
    ...
    function getDialogHtml(_html) {
        return '<div style="padding-bottom: 120px;">'+ _html +'</div>'
    }
    ...
    function eventHandle() {
        // 查看审批详情
        $(document).on('click', '.openApprovalDialog', function () {
            var $o = $(this),
                url = $o.attr('data-url');
            Req.getReq(url, function (res) {
                Dialog.confirmDialog({
                    title: '查看审批详情',
                    content: getDialogHtml(res),
                    area: ['1000px', '600px'],
                    btn: [],
                    success: function (layero, index) {
                        if(typeof userUuId != 'undefined' && typeof waterOpen != 'undefined' && userUuId && waterOpen) {
                            Common.loadWaterMark(userUuId, waterOpen);
                        }
                    }
                })
            }, 'html');
    ...
    // 审批详情
    function viewApprovalInfoInit() {
        loadDeptMemberList();
        eventHandle();
    }

    // 注意，这里是模块输出的核心，模块名必须和use时的模块名一致
    exports('Approval', viewApprovalInfoInit);
});